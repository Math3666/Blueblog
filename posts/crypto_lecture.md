...
a = p\_{1} \cdot p\_{2} \cdot p\_{3} \cdot \cdots \cdot p\_{t}

$$

其中 $p\_{1},p\_{2},p\_{3},\dots,p\_{t}$ 是素数。
...
假设 $t\_{1} \neq t\_{2}\ (0 \le t\_{1}, t\_{2} \le n-1)$，使得 $t\_{1}m + k \equiv t\_{2}m + k \pmod{n}$。

化简得 $(t\_{1} - t\_{2})m \equiv 0 \pmod{n}$。因 $\gcd(m, n) = 1$，故 $t\_{1} - t\_{2} \equiv 0 \pmod{n}$，即 $t\_{1} = t\_{2}$，矛盾。
...
假设整数 $m\_{1}, m\_{2}, \dots, m\_{r}$ 两两互素，则对于任意整数 $a\_{1}, a\_{2}, \dots, a\_{r}$，以下同余方程组有唯一解：
...
M = \prod\_{i=1}^{r} m\_{i} = m\_{1} m\_{2} \cdots m\_{r}
...
令 $M\_{i}^{-1}$ 为 $M\_{i}$ 模 $m\_{i}$ 的乘法逆元（即 $M\_{i} \cdot M\_{i}^{-1} \equiv 1 \pmod{m\_{i}}$）。

方程组在模 $M$ 下的唯一解为：

$$

x \equiv \sum\_{i=1}^{r} a\_{i} M\_{i} M\_{i}^{-1} \pmod{M}
