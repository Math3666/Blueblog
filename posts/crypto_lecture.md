...
> 本文档根据讲座幻灯片整理，涵盖数论基础定理、对称加密、RSA 公钥加密与 DSA 数字签名四大部分。
...
$$a = p_1 \cdot p_2 \cdot p_3 \cdot \cdots \cdot p_t$$
...
#### 步骤 1：通过欧几里得算法求 $\gcd(15, 21)$

核心逻辑：用较大数除以较小数，替换为"除数"与"余数"的组合，直到余数为 0，此时的除数即为 GCD。

- 第 1 步：$21 = 15 \times 1 + 6$（余数 $r_1 = 6 \neq 0$，继续）
- 第 2 步：$15 = 6 \times 2 + 3$（余数 $r_2 = 3 \neq 0$，继续）
- 第 3 步：$6 = 3 \times 2 + 0$（余数 $r_3 = 0$，停止）
...
#### 欧拉函数积性证明（$\varphi(mn) = \varphi(m)\varphi(n)$，其中 $\gcd(m,n)=1$）
...
假设存在 $t_1 \neq t_2\ (0 \le t_1, t_2 \le n-1)$，使得 $t_1m + k \equiv t_2m + k \pmod{n}$。

化简得 $(t_1 - t_2)m \equiv 0 \pmod{n}$。因 $\gcd(m, n) = 1$，故 $t_1 - t_2 \equiv 0 \pmod{n}$，即 $t_1 = t_2$，矛盾。
...
假设整数 $m_1, m_2, \dots, m_r$ 两两互素，则对于任意整数 $a_1, a_2, \dots, a_r$，以下同余方程组有唯一解：
...
2. $M_1 = 60/5 = 12$，求 $12$ 模 $5$ 的逆元：$12 \equiv 2 \pmod{5}$，$2 \times 3 = 6 \equiv 1 \pmod{5}$，故 $M_1^{-1} = 3$，对应项 $36 \times 2$
3. $M_2 = 60/4 = 15$，求 $15$ 模 $4$ 的逆元：$15 \equiv 3 \pmod{4}$，$3 \times 3 = 9 \equiv 1 \pmod{4}$，故 $M_2^{-1} = 3$，对应项 $45 \times 3$
4. $M_3 = 60/3 = 20$，求 $20$ 模 $3$ 的逆元：$20 \equiv 2 \pmod{3}$，$2 \times 2 = 4 \equiv 1 \pmod{3}$，故 $M_3^{-1} = 2$，对应项 $40 \times 0$

$$a = 36 \times 2 + 45 \times 3 + 40 \times 0 = 72 + 135 + 0 = 207$$

$207 \equiv 207 - 3 \times 60 = 207 - 180 = 27 \pmod{60}$
...
### 3.3 线性同余生成器（LCG, Linear Congruential Generator）
...
### 3.4 反馈移位寄存器（FSR, Feedback Shift Register）

结构：由 $n$ 个寄存器 $a_{n-1}, a_{n-2}, \dots, a_0$ 组成，输出序列 $\underline{a} = a_0 a_1 a_2 \cdots$，反馈函数为 $F(x_1, x_2, \dots, x_n)$。

- 新生成的信息与当前的状态相关
- 即：

$$a_n = F(a_0, a_1, a_2, \dots, a_{n-2}, a_{n-1})$$



### 3.5 线性反馈移位寄存器（LFSR, Linear Feedback Shift Register）

$F$ 是线性函数，即：

$$a_n = \sum_{i=0}^{n-1} c_i a_i$$
...
$$c^d \equiv m^{ed} \equiv m^{1+k\varphi} \equiv m \times (m^\varphi)^k \pmod{n}$$
...
$$m^{ed} \equiv m \times (m^\varphi)^k \equiv m \pmod{n}$$
...
很自然的，把 (1) 代入 (2)，算出来的 $r_0$ 和 $r$ 如果一致，那就说明参数都没受到影响，明文也就是可信的了。
...
$$u_1 \equiv g^{(H(m))s^{-1}} \pmod{q}$$

$$u_2 \equiv y^{rs^{-1}} \pmod{q}$$

然后比对是否有：

$$r \equiv (u_1 \times u_2 \bmod p) \bmod q$$

若等式成立，则签名验证通过，明文可信。



> **整理说明**：本文档完整覆盖了原讲座 37 张幻灯片的全部内容，包括数论基础定理（裴蜀定理、费马小定理、欧拉定理、中国剩余定理）、对称加密（流密码、PRNG、LCG、FSR、LFSR）、RSA 公钥加密（密钥生成、加解密、正确性证明、安全性与漏洞）以及 DSA 数字签名（参数、签名、验签）四大部分。所有数学公式均已核对原文。
